...
const SRC_TEMPLATE = `export const systemSounds: Record<number, Uint8Array> = {
    %%ENTRIES%%
};`;
...
if (require.main === module) {
  const [_tsNode, _dumpHeader, systemSoundDir, outPath] = process.argv;

  if (!systemSoundDir || !outPath) {
    console.error(
      "usage: ts-node bundleSystemSounds.ts <system-bg-dir> <out-path>"
    );
    process.exit(1);
  }

  main(path.resolve(systemSoundDir), path.resolve(outPath))
